...
const startTimer = () => {
  timer = setInterval(() => {
    if (state.get().TOTAL_TIME === 0) {
      stopTimer();
      showScreen(getResultTimeExpiredScreen(onGetNextLevel).element);
      initializeGame();
      return;
    }
    tick();
...
export const onGetNextLevel = () => {
  ...
  if (userResult.mistakes === Game.MISTAKES_COUNT) {
    stopTimer();
    showScreen(getResultAttemptExpiredScreen(onGetNextLevel).element);
    initializeGame();
    return;
  }
  ...
  if (level) {
    ...
    switch (level.type) {
      case Game.TYPES.GENRE:
        showScreen(getGenreScreen(level, state, onGetNextLevel).element);
        break;
      case Game.TYPES.ARTIST:
        showScreen(getArtistScreen(level, state, onGetNextLevel).element);
        break;
    }
    return;
  }
